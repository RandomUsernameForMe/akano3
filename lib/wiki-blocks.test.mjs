...
test(":::karta se rozpozná i s nadpisem", () => {
  const blocks = parseBlocks(":::karta Nultá generace\nZabíječi monster.\n:::")
  assert.equal(blocks.length, 1)
  assert.equal(blocks[0].type, "card")
  assert.equal(blocks[0].title, "Nultá generace")
  assert.deepEqual(blocks[0].lines, ["Zabíječi monster."])
})

test("karta bez nadpisu se nezpracuje jako karta", () => {
  const blocks = parseBlocks(":::karta\nText.\n:::")
  assert.equal(blocks[0].type, "md")
})

test("karty a tajemství se nepletou", () => {
  const blocks = parseBlocks(":::karta A\nX\n:::\n:::k4\nY\n:::")
  assert.equal(blocks.length, 2)
  assert.equal(blocks[0].type, "card")
  assert.equal(blocks[1].type, "secret")
  assert.equal(blocks[1].requiredLevel, 4)
})
